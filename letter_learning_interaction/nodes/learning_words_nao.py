...
SHAPE_TOPIC = rospy.get_param('trajectory_output_topic', '/write_traj')
...
SHAPE_TOPIC_DOWNSAMPLED = rospy.get_param(
    'trajectory_output_nao_topic', '/write_traj_downsampled')  # Name of topic to publish shapes to
# path to a log file where all learning steps will be stored
SHAPE_LOGGING_PATH = rospy.get_param('shape_log', '')

# tablet params
# Waits for the button of finish in the tablet to be pressed
SHAPE_FINISHED_TOPIC = rospy.get_param(
...
# Listen for user shapes -Currently not used-
...
TEST_TOPIC = rospy.get_param('test_request_topic', 'test_learning')
# Listen for when stop card has been shown to the robot
STOP_TOPIC = rospy.get_param('stop_request_topic', 'stop_learning')
NB_REPETITIONS_TOPIC = rospy.get_param(
    'nb_repetitions_topic', 'nb_repetitions')
GRADE_TOPIC = rospy.get_param('grade_topic', "user_feedback")
SIMPLE_LEARNING_PACE_TOPIC ='simple_learning_pace'
    ...
    if(stateMachine.get_state() == "WAITING_FOR_FEEDBACK"
       or stateMachine.get_state() == "ASKING_FOR_FEEDBACK"):
        ...
        if demo_from_template:
            rospy.loginfo(
                'Received template demonstration for letters ' + str(demo_from_template.keys()))
            ...
            if activeLetter:
                ...
                activeLetter = None
                rospy.loginfo(
                    'Received demonstration for selected letter ' + shape.shapeType)
            else:
                ...
                if letter:
                    ...
                    rospy.loginfo(
                        'Received demonstration for ' + shape.shapeType)
                else:
                    rospy.logwarn(
                        'Received demonstration, but unable to find the letter that was demonstrated! Ignoring it.')
                    return

            # replace any existing feedback with new
            demoShapesReceived = [shape]
...
def onClearScreenReceived(message):
    ...
    try:
        clear_all_shapes = rospy.ServiceProxy(
            'clear_all_shapes', clearAllShapes)
...
def onSimpleLearningPaceReceived(message):
    global simpleLearningPace
    simpleLearningPace = message.data
    rospy.loginfo('Simple Learning Pace set to ' + str(message.data ))
...
def onNewChildReceived(message):
    ...
    if naoSpeaking:
        if alternateSidesLookingAt:
            lookAndAskForFeedback(introJokePhrase, nextSideToLookAt, naoWriting,
                                  naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
        else:
            lookAndAskForFeedback(introJokePhrase, personSide, naoWriting, naoSpeaking,
                                  textToSpeech, motionProxy, armJoints_standInit, effector)
    # clear screen
...
def onSetActiveShapeGesture(message):
    ...
    activeLetter, bb = screenManager.closest_letter(
...
def respondToDemonstration(infoFromPrevState):
    ...
    for shape_msg in demoShapesReceived:
        ...
        new_shape_msg, score = learningManager.respond_to_demonstration_letter(
            glyph, shapeName, 'simple',simpleLearningPace)
    ...
    infoForNextState = {'state_goTo': state_goTo,
                        'state_cameFrom': "RESPONDING_TO_DEMONSTRATION", 'shapesToPublish': new_shapes}
    # TODO PUB SCORE
...
def respondToDemonstrationWithFullWord(infoFromPrevState):
    ...
    for shape_msg in demoShapesReceived:
        ...
        glyph = downsampleShape(glyph, NUMPOINTS_SHAPEMODELER)
        rospy.loginfo("Downsampling of %s done. Demo received for %s" %
                      (shapeName, shapeName))
        new_shape_msg, score = learningManager.respond_to_demonstration_letter(
            glyph, shapeName, 'simple', simpleLearningPace)
...
def publishShape(infoFromPrevState):
    ...
    # publish next remaining shape (and remove from list)
    shape = shapesToPublish.pop(0)

    try:
        display_new_shape = rospy.ServiceProxy(
            'display_new_shape', displayNewShape)
    ...
    headerString = shape.shapeType + '_' + \
        str(shape.paramsToVary) + '_' + str(shape.paramValues)

    traj_downsampled = make_traj_msg(
        shape.path, shapeCentre, headerString, t0, True, dt)  # for robot

    traj = make_traj_msg(shape.path, shapeCentre, headerString,
                         t0, False, float(dt) / DOWNSAMPLEFACTOR)
    ...
    infoForNextState = {'state_cameFrom':  "PUBLISHING_LETTER", 'state_goTo': [
        "ASKING_FOR_FEEDBACK"], 'centre': trajStartPosition, 'shapePublished': shape.shapeType}  # only appends most recent shape's info (@TODO)
    if(len(shapesToPublish) > 0):  # more shapes to publish
        # come back to publish the remaining shapes
        state_goTo = deepcopy(drawingLetterSubstates)
        infoForNextState = {'state_goTo': state_goTo, 'state_cameFrom': "PUBLISHING_LETTER",
                            'shapesToPublish': shapesToPublish, 'centre': trajStartPosition}
...
def publishWord(infoFromPrevState):
    ...
    traj = make_traj_msg(placedWord, float(
        dt) / DOWNSAMPLEFACTOR, FRAME, delayBeforeExecuting, t0, pen_ups,  log=True)
    ...
    downsampledTraj = make_traj_msg(
        downsampledShapedWord, dt, FRAME, delayBeforeExecuting, t0, pen_ups,  log=True)
    ...
    nextState = "WAITING_FOR_LETTER_TO_FINISH"
    infoForNextState = {'state_cameFrom':  "PUBLISHING_WORD", 'state_goTo': [
        "ASKING_FOR_FEEDBACK"], 'centre': trajStartPosition, 'wordWritten': infoFromPrevState['wordToWrite']}

    return nextState, infoForNextState


infoToRestore_waitForShapeToFinish = rospy.Subscriber(
    SHAPE_FINISHED_TOPIC, String, onShapeFinished)
...
def waitForShapeToFinish(infoFromPrevState):
    ...
    if shapeFinished:
        ...
        ref_boundingboxes = screenManager.place_reference_boundingboxes(
            learningManager.current_word)
        for bb in ref_boundingboxes:
            pub_bounding_boxes.publish(
                make_bounding_box_msg(bb, selected=False))
...
def respondToNewWord(infoFromPrevState):
    ...
    nextState = 'PUBLISHING_WORD'
    infoForNextState = {'state_cameFrom': "RESPONDING_TO_NEW_WORD",
                        'shapesToPublish': shapesToPublish, 'wordToWrite': wordToLearn}
...
def askForFeedback(infoFromPrevState):
    ...
    if infoFromPrevState['state_cameFrom'] == "PUBLISHING_WORD":
        ...
        if naoSpeaking:
            ...
            if(alternateSidesLookingAt):
                lookAndAskForFeedback(toSay, nextSideToLookAt, naoWriting, naoSpeaking,
                                      textToSpeech, motionProxy, armJoints_standInit, effector)
                ...
                lookAndAskForFeedback(toSay, personSide, naoWriting, naoSpeaking,
                                      textToSpeech, motionProxy, armJoints_standInit, effector)
...
def pauseInteraction(infoFromPrevState):
    ...
    if changeActivityReceived == 'learning_words_nao':
        ...
        if naoSpeaking:
            if(alternateSidesLookingAt):
                lookAndAskForFeedback(againLearningWordsPhrase, nextSideToLookAt, naoWriting,
                                      naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
                rospy.sleep(1)
                lookAndAskForFeedback(introLearningWordsPhrase, personSide, naoWriting,
                                      naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
            else:
                lookAndAskForFeedback(againLearningWordsPhrase, nextSideToLookAt, naoWriting,
                                      naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
...
def startInteraction(infoFromPrevState):
    ...
    if changeActivityReceived == 'learning_words_nao':
        ...
        if naoSpeaking:
            if(alternateSidesLookingAt):
                lookAndAskForFeedback(introLearningWordsPhrase, nextSideToLookAt, naoWriting,
                                      naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
            else:
                lookAndAskForFeedback(introLearningWordsPhrase, personSide, naoWriting,
                                      naoSpeaking, textToSpeech, motionProxy, armJoints_standInit, effector)
...
if __name__ == "__main__":
    ...
    if (datasetDirectory.lower() == 'default'):  # use default
        ...
        datasetDirectory = installDirectory + \
            '/share/allograph/letter_model_datasets/bad_letters'
        robotDirectory = installDirectory + '/share/allograph/robot_tries/start'
        rospy.loginfo('datasetDirectory :'+datasetDirectory)
        rospy.loginfo('robotDirectory :'+robotDirectory)


    stateMachine = StateMachine()
    stateMachine.add_state("STARTING_INTERACTION", startInteraction)
    stateMachine.add_state(
        "WAITING_FOR_ROBOT_TO_CONNECT", waitForRobotToConnect)
    ...
    stateMachine.add_state(
        "WAITING_FOR_LETTER_TO_FINISH", waitForShapeToFinish)
    stateMachine.add_state("ASKING_FOR_FEEDBACK", askForFeedback)
    stateMachine.add_state("WAITING_FOR_FEEDBACK", waitForFeedback)
    stateMachine.add_state("RESPONDING_TO_DEMONSTRATION",
                           respondToDemonstration)
    stateMachine.add_state(
        "RESPONDING_TO_DEMONSTRATION_FULL_WORD", respondToDemonstrationWithFullWord)
    stateMachine.add_state("RESPONDING_TO_TEST_CARD", respondToTestCard)
    stateMachine.add_state(
        "WAITING_FOR_TABLET_TO_CONNECT", waitForTabletToConnect)
    ...
    clear_subscriber = rospy.Subscriber(
        CLEAR_SURFACE_TOPIC, Empty, onClearScreenReceived)
    # listen for test time
    test_subscriber = rospy.Subscriber(
        TEST_TOPIC, Empty, onTestRequestReceived)
    # listen for when to stop
    stop_subscriber = rospy.Subscriber(
        STOP_TOPIC, Empty, onStopRequestReceived)
    # listen for user-drawn shapes
    shape_subscriber = rospy.Subscriber(
        PROCESSED_USER_SHAPE_TOPIC, ShapeMsg, onUserDrawnShapeReceived)
    ...
    change_activity_subscriber = rospy.Subscriber(
        ACTIVITY_TOPIC, String, onChangeActivity)
    # listen for feedback
    grade_subscriber = rospy.Subscriber(GRADE_TOPIC, String, onGradeReceived)
    simpleLearningPace_subscriber = rospy.Subscriber(SIMPLE_LEARNING_PACE_TOPIC, Float32, onSimpleLearningPaceReceived)
    ...
    from letter_learning_interaction.watchdog import Watchdog
    ...
    rospy.loginfo("Nao configuration: writing=%s, speaking=%s (%s), standing=%s, handedness=%s" % (
        naoWriting, naoSpeaking, LANGUAGE, naoStanding, NAO_HANDEDNESS))

    myBroker, postureProxy, motionProxy, textToSpeech, armJoints_standInit = ConnexionToNao.setConnexion(
        naoConnected, naoWriting, naoStanding, NAO_IP, LANGUAGE, effector)
    ...
    learningManager = LearningManager(
        datasetDirectory, datasetDirectory, robotDirectory, datasetDirectory,'simple',0.5)
    rospy.loginfo("reference letters list of length %i", len(learningManager.refs))
