...
class log():
    ...
    def __init__(self):
        ...
        # The number of word repetitions
        rospy.Subscriber("nb_repetitions", Int16, self.repetitions_callback)
        # Response time till the child writes
        rospy.Subscriber("time_response", Float32, self.response_callback)
        # Writing time during demostration
        rospy.Subscriber("time_writing", Float32, self.writing_callback)

        # Current activity
        rospy.Subscriber("activity", String, self.activity_callback)
        # Word to be written
        rospy.Subscriber("words_to_write", String, self.word_callback)
        # New child in da house
        rospy.Subscriber("new_child", String, self.child_callback)

        rospy.Subscriber("current_demo", ShapeMsg,
                         self.demo_callback)     # Path of the demo
        rospy.Subscriber("current_learn", ShapeMsg,
                         self.learn_word_callback)     # Path of the learn
        rospy.Subscriber("current_score", Float32,
                         self.score_callback)     # Score

        rospy.Subscriber("clear_screen", Empty,
                         self.clearscreen_callback)     # clearscreen
        rospy.Subscriber("user_feedback", String,
                         self.userfeedback_callback)     # user_feedback
        rospy.Subscriber("stop_learning", Empty,
                         self.close)     # user_feedback
